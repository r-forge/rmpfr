\documentclass[article]{jss}% for JSS, shorter
...
% https://tex.stackexchange.com/questions/19359/reduce-space-between-sinput-and-soutput
\newlength{\FVtopsep}
...
options(width = 75,
	...
	## otherwise, MM wants
	useFancyQuotes = "TeX"
	## prompt="> ", continue="  " # 2 (or 3) blanks: use same length as 'prompt'
	)
...
\citet{DidAM92}\footnote{In the Fortran source, file ``\code{708}'', also
  available as \url{https://www.netlib.org/toms/708},
...
so their difference suffers from cancellation, losing accuracy.
%% 2
...
automatically\footnote{look for ``log(1-exp(x))'' in
  \url{https://svn.r-project.org/R/branches/R-1-9-patches/src/nmath/dpq.h}}.
...
As can be seen in Figure~\ref{fig:bigpic}, details in \cite{MaeM12-Rmpfr-log1mexp},
%% FIXME "cache = TRUE": ---
<<comp-big, echo=FALSE>>=
...
@
%This is revealing: N%
neither method, log1p or expm1, is uniformly good enough, and for very
large $a$, all three double precision methods give 0, \emph{and} that is
the best approximation in double precision and we can restrict ourselves to
smaller $a$, i.e., smaller than about 710.% < 709.78271289338403 (lynne 64b)
...
Further note that for really small $a$'s,
%% <<a.small>>=
%% t3.l1e(1e-20)
...
<<bigpict-def, eval=FALSE, echo=FALSE>>=
...
<<boxcox, echo=FALSE, results=hide>>=
...
@
The optimal boxcox exponent turns out to be close to $\frac 1 3$.
Then, the crossover point of the two curves already suggests that the
cutoff, $a_0 = \log 2$ is empirically very close to optimal.
<<zoom-in-def-1, eval=FALSE, echo=FALSE>>=
...
\end{align}
for $x\to\infty$.
%%>>>--- here have more in ./log1mexp-note.Rnw <<<
...
save computations.\footnote{The
  \R{} plot \code{curve(log1p(exp(x)) - x, 33.1, 33.5, n=2\^{}10)}
  reveals
  a somewhat fuzzy cutoff $x_2$.}
...
% increasing width --> effective LaTeX *height* will decrease
\setcapwidth{\textwidth}%
  \caption{Relative errors$^{*}$ % \footnote{(see footnote of Fig.~\ref{fig:bigpic})}
...
where $e_{\mathrm{Max}}$ is the maximal exponent without overflow,
$e_{\mathrm{Max}} \approx $
\Sexpr{round(.Machine[["double.max.exp"]] * log(2), 1)} \ $=$ \
\code{.Machine\$double.max.exp * log(2)}.
...
Note that an extended version of this paper is available as
vignette (in \texttt{Sweave}, i.e., with complete \R{} source,
in \R{} package \pkg{Rmpfr}, as \citet{MaeM12-Rmpfr-log1mexp}.

%% \subsection*{Session Information}
%% \nopagebreak
<<sessionInfo, results=hide, echo=FALSE>>=
