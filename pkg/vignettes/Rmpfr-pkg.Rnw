...
\Abstract{
 ...
 \href{https://bugs.R-project.org/show_bug.cgi?id=14491}{\code{PR\#14491}}.
}
...
see \url{https://mpfr.org/} and \cite{FouLHLPZ:2007} and the
...
\url{https://gmplib.org/} and \cite{GMP:2011}. It can be obtained from there,
...
\code{"catalan"} for
Catalan's constant (see \url{https://en.wikipedia.org/wiki/Catalan\%27s_constant}).
<<Const-ex>>=
Const("pi")
Const("log2")
@
...
\end{equation*}
see \url{https://en.wikipedia.org/wiki/Polylogarithm#Dilogarithm}.
